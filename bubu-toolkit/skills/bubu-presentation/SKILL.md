...
- **Voice: follow the `bubu-stopslop` skill** (`bubu-toolkit/skills/bubu-stopslop/SKILL.md`), the single source of truth for banned language, cadence, and the self-edit pass. No em-dashes (—) and no AI-sounding language anywhere: headlines, body copy, speaker notes, captions.
- **Slide-specific:** headlines are assertions, not labels ("Kahf owns the street", never "Market Overview"). One idea per slide. Declarative, confident, specific; every line written by a strategist who knows the room.
